...
int main()
{
    ...
    Shader planeShader((SRC_PATH + (std::string)"shadow_mapping.vert").c_str(),
        (SRC_PATH + (std::string)"shadow_mapping.frag").c_str());

    // depth shaders
    Shader simpleDepthShader((SRC_PATH + (std::string)"shadow_mapping_depth.vert").c_str(),
        (SRC_PATH + (std::string)"shadow_mapping_depth.frag").c_str());

	// skybox shader
    Shader skyboxShader((SRC_PATH + (std::string)"skybox.vert").c_str(),
        (SRC_PATH + (std::string)"skybox.frag").c_str());
    ...
    glm::vec3 planePos(0.0f, -4.0f, 0.0f);
    ...
    {
        SRC_PATH + (std::string)"night-sky-cube-map/right.png",
        SRC_PATH + (std::string)"night-sky-cube-map/left.png",
        SRC_PATH + (std::string)"night-sky-cube-map/top.png",
        SRC_PATH + (std::string)"night-sky-cube-map/bottom.png",
        SRC_PATH + (std::string)"night-sky-cube-map/front.png",
        SRC_PATH + (std::string)"night-sky-cube-map/back.png"
    };
    unsigned int cubemapTexture = loadCubemap(faces);

    unsigned int diffuseMap = loadTexture((SRC_PATH + (std::string)"container2.png").c_str());
    unsigned int specularMap = loadTexture((SRC_PATH + (std::string)"container2_specular.png").c_str());
...
void processInput(GLFWwindow* window)
{
	...
	// toggle camera/cursor mode
    if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS)
    {
        if (camera.mode == CURSOR)
        {
            glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
            camera.mode = LOOK_AROUND;
        }
        else
        {
...
void drawIMGUI() {
    ...
    // create a named window.
    {
        ...
        ImGui::Begin("G4G Midterm Project 2022");  // Create a window and append into it.
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < 0; i++)
            // don't draw any point lights. left here for experimentation.
        {
            model = glm::mat4(1.0f);
            model = glm::translate(model, sin((float)glfwGetTime()) * pointLightPositions[i]);
            model = glm::scale(model, glm::vec3(0.2f));
...
int main()
{
    ...
    // glfw window creation
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Graphics4Games Arch 2022", NULL, NULL);
    if (window == NULL)
    {
        std::cout << "Failed to create GLFW window" << std::endl;
        glfwTerminate();
    ...
    drawCircle(eye1, .1, myColor);
    drawCircle(eye1, .03, myColor);

    drawCircle(eye2, .1, myColor);
    drawCircle(eye2, .03, myColor);
...
int main()
{
    ...
    float vertices[] = {
        // position           // color
         -0.5f,  0.5f, 0.0f,  0.5f, 0.25f, 0.75f,  // left
          0.5f,  0.5f, 0.0f,  0.25f, 0.5f, 0.75f,  // right
          0.0f, -0.5f, 0.0f,  0.25f, 0.75f, 0.5f   // top
    };
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float moveValue = sin(timeValue) / 2.0f;
        int vertexMoveLocation = glGetUniformLocation(shaderProgram, "addPos");
        glUniform3f(vertexMoveLocation, moveValue, moveValue, 0.0f);
...
int main()
{
    ...
    // build and compile our shader program
    // ------------------------------------
    Shader ourShader(SRC_PATH + "shader.vert", SRC_PATH + "shader.frag"); // you can name your shader files however you like
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < 10; i++)
        {
            ...
            float angle = 20.0f * i;
            model = glm::rotate(model, (float)glfwGetTime(), glm::vec3(0.5f * sinf(i), 1.0f * cosf(i), 0.0f));
            ourShader.setMat4("model", model);
...
int main()
{
    ...
    // build and compile our shader zprogram
    // ------------------------------------
    Shader lightingShader((SRC_PATH + (std::string)"lighting.vert").c_str(), 
        (SRC_PATH + (std::string)"lighting.frag").c_str());
    Shader lightCubeShader((SRC_PATH + (std::string)"lighting.vert").c_str(),
        (SRC_PATH + (std::string)"lighting2.frag").c_str());
...
int main()
{
    ...
    // -----------------------------------------------------------------------------
    unsigned int diffuseMap = loadTexture((SRC_PATH + (std::string)"hoi3@rpi.edu.png").c_str());
    unsigned int specularMap = loadTexture((SRC_PATH + (std::string)"container2_specular.png").c_str());
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        */
        ...
        for (unsigned int i = 0; i < 4; i++)
        {
            model = glm::mat4(1.0f);
            model = glm::translate(model, sin((float)glfwGetTime()) * pointLightPositions[i]);
            model = glm::scale(model, cos((float)glfwGetTime()) * glm::vec3(0.2f)); // Make it a smaller cube
            lightCubeShader.setMat4("model", model);
            glDrawArrays(GL_TRIANGLES, 0, 36);
        }
...
int main()
{
    ...
    // build and compile shaders
    // -------------------------
    Shader shader((SRC_PATH + (std::string)"shadow_mapping.vert").c_str(), 
        (SRC_PATH + (std::string)"shadow_mapping.frag").c_str());
    Shader simpleDepthShader((SRC_PATH + (std::string)"shadow_mapping_depth.vert").c_str(), 
        (SRC_PATH + (std::string)"shadow_mapping_depth.frag").c_str());
    Shader debugDepthQuad((SRC_PATH + (std::string)"debug_quad_depth.vert").c_str(), 
        (SRC_PATH + (std::string)"debug_quad_depth.frag").c_str());
